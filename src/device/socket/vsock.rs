...
mod tests {
    ...
    struct SocketFixture {
        driver: VirtIOSocket<FakeHal, FakeTransport<VirtioVsockConfig>>,
        state: Arc<Mutex<State<VirtioVsockConfig>>>,
    }

    fn socket() -> SocketFixture {
        ...
        SocketFixture {
            driver: VirtIOSocket::new(transport).unwrap(),
            state,
        }
    }
    ...
    fn failed_submission_preserves_credit_and_observation() {
        ...
        let SocketFixture { mut driver, state } = socket();
